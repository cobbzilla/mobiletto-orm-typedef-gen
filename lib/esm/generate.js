...
import { isArrayType, MobilettoOrmError, MobilettoOrmTypeDef, } from "mobiletto-orm-typedef";
...
const defaultPrepareContext = (typeDef, ctx) => {
    ...
    if (typeDef.search && typeDef.search.refSearch) {
        ...
        if (ctx.hasRefSearch) {
            const refSearches = Object.keys(refSearch)
                ...
                return ({
                    ref,
                    Ref: capitalize(ref),
                    refType: ((_a = typeDef.fields[ref].ref) === null || _a === void 0 ? void 0 : _a.refType) || "?refType?",
                    RefType: capitalize(((_b = typeDef.fields[ref].ref) === null || _b === void 0 ? void 0 : _b.refType) || "?RefType?"),
...
const validTemplateFile = (path) => !!VALID_TEMPLATE_FILE_EXT.find((ext) => path.endsWith(ext));
export const generate = (typeDef, templatePath, opts, ctx) => {
    ...
    const mobilettoOrmObjectPackage = (opts === null || opts === void 0 ? void 0 : opts.mobilettoOrmObjectPackage) || "mobiletto-orm-typedef";
    const disclaimer = (opts === null || opts === void 0 ? void 0 : opts.disclaimer) ? opts.disclaimer : STANDARD_AUTOGEN_FILE_DISCLAIMER;
    const rootOnly = (opts === null || opts === void 0 ? void 0 : opts.rootOnly) === true;
    ...
    while (decls.length > 0) {
        ...
        if (!decl)
            break;
        const typeContext = defaultPrepareContext(typeDef, Object.assign({ typeDef, name: decl.name, Name: capitalize(decl.name), fields: decl.fields, fieldNames: Object.keys(decl.fields), disclaimer: decls.length > 0 ? null : disclaimer, header: decls.length > 0 ? null : opts === null || opts === void 0 ? void 0 : opts.header, first, root: decl.root || false, typescript,
            mobilettoOrmObjectPackage }, ctx));
        ...
        const endPath = templatePath.substring(0, lastSlash).replace(/[^A-Z0-9_./-]/gi, "") + templatePath.substring(lastSlash);
